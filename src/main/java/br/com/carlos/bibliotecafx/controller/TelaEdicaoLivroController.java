...
public class TelaEdicaoLivroController {
    ...
    @FXML private Button btnCarregarCapa;
    ...
    private void carregarNovaCapa() {
        ...
        File arquivoSelecionado = fileChooser.showOpenDialog(btnCarregarCapa.getScene().getWindow());
    ...
    private void salvarAlteracoes() {
        ...
        livroAtual.setAno(txtAno.getText().trim().isEmpty() ? 0 : Integer.parseInt(txtAno.getText().trim()));
        livroAtual.setNumeroPaginas(txtPaginas.getText().trim().isEmpty() ? 0 : Integer.parseInt(txtPaginas.getText().trim()));
    ...
    private boolean validarCampos() {
        ...
        try {
            if (!txtAno.getText().trim().isEmpty()) Integer.parseInt(txtAno.getText().trim());
        } catch (NumberFormatException e) {
            mostrarAlertaErro("Erro de Validação", "O campo 'Ano' deve ser um número válido.");
            return false;
        }
        try {
            if (!txtPaginas.getText().trim().isEmpty()) Integer.parseInt(txtPaginas.getText().trim());
        } catch (NumberFormatException e) {
